...
DWORD WINAPI ThreadLer(LPVOID param) {
    ThreadDadosReadOperador* dados = (ThreadDadosReadOperador*)param;
    TCHAR* msg = nullptr;
...
DWORD WINAPI ThreadEscrever(LPVOID param) {
    ThreadDadosWriteOperador* dados = (ThreadDadosWriteOperador*)param;
    TCHAR msg[NUM_CHARS];
    std::wstring startInput;
    HANDLE hSemO = CreateSemaphore(NULL, 0, TAM, NOME_SEMOCUPADOS);  // hSemO = OpenSemaphore();
    HANDLE hSemL = CreateSemaphore(NULL, TAM, TAM, NOME_SEMLIVRES);
    while (!(dados->terminar)) {
        ...
        WaitForSingleObject(hSemL, INFINITE);   // hSemLivres (--)
        WaitForSingleObject(dados->hMutex, INFINITE);
